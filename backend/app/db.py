...
async def create_schema(session_factory: async_sessionmaker[AsyncSession]) -> None:
    ...
    async with engine.begin() as connection:
        ...
        await connection.execute(text("CREATE TABLE IF NOT EXISTS user_vm_connections (id SERIAL PRIMARY KEY, user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, vm_id INTEGER NOT NULL REFERENCES vm_assets(id) ON DELETE CASCADE, guacamole_connection_id VARCHAR(160) NOT NULL, connection_name VARCHAR(160) NOT NULL, created_by INTEGER REFERENCES users(id), created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP, CONSTRAINT uq_user_vm_connection UNIQUE(user_id,vm_id))"))
        await connection.execute(text("CREATE INDEX IF NOT EXISTS ix_user_vm_connections_user_id ON user_vm_connections(user_id)"))
        await connection.execute(text("CREATE INDEX IF NOT EXISTS ix_user_vm_connections_vm_id ON user_vm_connections(vm_id)"))
